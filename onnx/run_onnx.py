...
def create_and_bind_ids(io_binding, bs, device, device_idx, dtype):
    ids = torch.randint(0, 1000, (bs, 128)).numpy()
    ...
    io_binding.bind_output('last_hidden_state')


def eval_latency(
    model_name, providers, iters=100, batch_sizes=[1], graph_opt=1,
    ...
    for bs in batch_sizes:
        ...
        if model_name == "bert":
            create_and_bind_ids(io_binding, bs, device, device_idx, dtype)
        else:
...
def main(args):
    ...
    results = eval_latency(
            args.model_name, providers, args.iters, batch_sizes, args.graph_opt, args.device, args.device_idx, dtype)
